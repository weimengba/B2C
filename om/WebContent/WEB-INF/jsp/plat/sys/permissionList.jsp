					...
					<div class="col-lg-12">
						...
						<div class="panel panel-default plain toggle panelClose panelRefresh">
							...
							<div class="panel-heading white-bg">
								<form style="margin-top: 20px;" class="common-page-form" action="plat/sys/permissionList.do" method="get">
									...
									<label>权限名：</label><input type="text" name="name" value="${param.name }">
									<label>URL：</label><input type="text" name="url" value="${param.url }">
									<input type="submit" value="查询">
								</form>
								<a class="bike-add-button btn btn-primary btn-sm pull-right" href="plat/sys/sysPermission.do">添加</a>
							</div>
							<div class="panel-body">
								<table class="table display table-bordered table-hover table-striped table-condensed" id="datatable">
									<thead>
										<tr>
											<th class="common-order-button" data-order="name" data-desc="">权限名</th>
											<th class="common-order-button" data-order="url" data-desc="">URL</th>
											<th>操作</th>
										</tr>
									</thead>
									...
									<tbody>
										<c:forEach items="${data.list }" var="d">
											<tr class="odd gradeX">
												<td>${d.name }</td>
												<td>${d.url }</td>
												<td class="center" width="12%"><a class="btn btn-primary btn-sm" href="plat/sys/sysPermission.do?id=${d.id }">编辑</a>|<a class="btn btn-primary btn-sm"
													href="plat/sys/sysPermissionDelete.do?id=${d.id }">删除</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
