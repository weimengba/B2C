			...
			<div class="row">
				...
				<div class="col-lg-12 heading">
					<h1 class="page-header">
						<c:if test="${object == null }">添加GoodsCategory</c:if>
						<c:if test="${object != null }">编辑GoodsCategory</c:if>
					</h1>
					<!-- Start .bredcrumb -->
					...
					<div class="col-lg-12">
						...
						<div class="panel panel-default toggle">
							...
							<div class="panel-heading">
								<h3 class="panel-title">添加或者修改商品分类</h3>
							</div>
							<div class="panel-body">
								<form action="plat/goodsCategory/goodsCategorySave.do" method="get" class="form-horizontal group-border hover-stripped" role="form" id="validate">
									...
									<div class="form-group">
										<label class="col-lg-2 control-label">分类级别</label>
										<div class="col-lg-10">
										
											<select class="col-lg-2 control-label" name="level">  
												   <c:if test="${object.level==null}">
													  <option value="0">请选择分类级别</option>
													 <option value="1">一级分类</option> 										     
												     <option value="2">二级分类</option>
												   </c:if>
												   <c:if test="${object.level==1}"> 
													 <option value="1">一级分类</option> 										     
												     <option value="2">二级分类</option>
												   </c:if>
												   <c:if test="${object.level==2}">  		     
												     <option value="2">二级分类</option>
												     <option value="1">一级分类</option> 
												   </c:if>
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											...
											<select class="col-lg-2 control-label" name="parentId">
													<c:forEach items="${goodsCategory }" var="goc">
														<option value="${goc.id }"
														<c:if test="${object.parent_id == goc.id}">
														selected="selected"
														</c:if >
														>
															${goc.name }
