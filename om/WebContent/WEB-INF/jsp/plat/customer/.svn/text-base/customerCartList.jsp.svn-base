					...
					<div class="col-lg-12">
						...
						<div class="panel panel-default plain toggle panelClose panelRefresh">
							...
							<div class="panel-heading white-bg">
								<form style="margin-top: 20px;" class="common-page-form" action="plat/customer/cartList.do" method="get">
									...
									<label>商品状态：</label>
									<input type="radio" name="isOnsale" value="1">已失效
									<input type="radio" name="isOnsale" value="2">未失效
								...
								<table class="table display table-bordered table-hover table-striped table-condensed" id="datatable">
									<thead>
										<tr>
											...
											<th class="common-order-button" data-order="num" data-desc="">商品数量</th>
											<th class="common-order-button" data-order="price" data-desc="">商品价格</th>
											<th class="common-order-button" data-order="isOnsale" data-desc="">商品状态</th>
											<th class="common-order-button" data-order="ctime" data-desc="">加入时间</th>
									...
									<tbody>
										<c:forEach items="${data.list }" var="d">
											<tr class="odd gradeX">
												...
												<td>${d.price }</td>
												<td>
												   <c:if test="${d.isOnsale==1}">
												                   已失效
                                                   </c:if>
                                                   <c:if test="${d.isOnsale==2}">
												                   未失效
                                                   </c:if>
												</td>
												<td>${d.ctime }</td>
