						...
						<div class="panel panel-default toggle">
							...
							<div class="panel-heading">
								...
								<h3 class="panel-title">
									<c:if test="${object == null }">添加商品属性值</c:if>
									<c:if test="${object != null }">编辑商品属性值</c:if>
								</h3>
							</div>
							<div class="panel-body">
								<form action="plat/goodsAttrValue/goodsAttrValueSave.do" method="get" class="form-horizontal group-border hover-stripped" role="form" id="validate">
									...
									<div class="form-group">
										<label class="col-lg-2 control-label">所属商品</label>
										<div class="col-lg-10">
											<select class="col-lg-2 control-label" name="goodsId" onchange="attrValue(this.value)">
													<c:forEach items="${goods }" var="g">
														<option value="${g.id }"
														<c:if test="${object.goods_id == g.id}">
														selected="selected"
														</c:if >
														>
															${g.title }
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<select class="col-lg-2 control-label" name="attrId" id="attr">
													
													 <%-- <c:forEach items="${goodsAttr }" var="ga">
														<option value="${ga.id }"
														<c:if test="${object.attr_id == ga.id}">
														selected="selected"
														</c:if >
														>
																${ga.title }
									...
									<div class="form-group">
										...
										<div class="col-lg-10">
											<input type="text" name="value" value="${object.value }" class="form-control">
										</div>
									</div>
							</div>
							<!-- End .form-group  -->
