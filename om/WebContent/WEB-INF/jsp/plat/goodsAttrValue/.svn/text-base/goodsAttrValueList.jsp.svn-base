					...
					<div class="col-lg-12">
						...
						<div class="panel panel-default plain toggle panelClose panelRefresh">
							...
							<div class="panel-heading white-bg">
								<form style="margin-top: 20px;" class="common-page-form" action="plat/goodsAttrValue/goodsAttrValueList.do" method="get">
									...
									<label>属性值：</label><input type="text" name="value" value="${param.value }">
									<input type="submit" value="查询">
								</form>
								<a class="bike-add-button btn btn-primary btn-sm pull-right" href="plat/goodsAttrValue/goodsAttrValue.do">添加</a>
							</div>
								...
								<table class="table display table-bordered table-hover table-striped table-condensed" id="datatable">
									<thead>
										<tr>
											...
											<th class="common-order-button" data-order="value" data-desc="">属性值</th>
											<th class="common-order-button" data-order="goodsId" data-desc="">所属商品</th>
											<th class="common-order-button" data-order="attrId" data-desc="">对应属性</th>
									...
									<tbody>
										<c:forEach items="${data.list }" var="d">
											<tr class="odd gradeX">
												<td>${d.id }</td>
												<td>${d.value }</td>
												<td width="5px">${d.goodsId }</td>
												<td>${d.attrId }</td>
												<td>${d.ctime }</td>
												<td>${d.utime }</td>
												<td class="center" width="12%"><a class="btn btn-primary btn-sm" href="plat/goodsAttrValue/goodsAttrValue.do?id=${d.id }">编辑</a>|<a class="btn btn-primary btn-sm"
													href="plat/goodsAttrValue/goodsAttrValueDelete.do?id=${d.id }">删除</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
