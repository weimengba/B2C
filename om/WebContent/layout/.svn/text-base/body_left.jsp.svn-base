...
<div id="sidebar">
	...
	<div class="sidebar-inner">
		...
		<ul id="sideNav" class="nav nav-pills nav-stacked">
			...
			<c:forEach items="${sys_menu}" var="menu1">
				<c:if test="${menu1.parent == 0 }">
					<li><a href="#"> ${menu1.name } <i class="${menu1.icon }"></i></a>
						<ul class="nav sub">
							<c:forEach items="${sys_menu}" var="menu2">
								<c:if test="${menu1.id == menu2.parent }">
									<li><a href="${menu2.url }"><i class="${menu2.icon }"></i> ${menu2.name }</a></li>
								</c:if>
							</c:forEach>
						</ul>
					<li>
